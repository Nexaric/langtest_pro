...
class InformalLetterLesson1 extends StatefulWidget {
  ...
  const InformalLetterLesson1({super.key, required this.lessonData});

  @override
  State<InformalLetterLesson1> createState() => _InformalLetterLesson1State();
}

class _InformalLetterLesson1State extends State<InformalLetterLesson1>
...
Hope you're doing great! I’m writing to let you know I’m throwing a birthday party next Saturday, and I’d love for you to come.

It’s going to be at my place on the 12th of May, starting around 6 p.m. There’ll be food, music, games, and a bunch of our old friends. I’ve even arranged for a karaoke setup—remember how much fun that was last time?
...
1. Structure:
   - Greeting: Use a friendly salutation (e.g., "Hi [Name]")
   - Opening: Start with a warm, casual tone
   - Body: Provide event details (date, time, place, activities)
   - Closing: Encourage a response, end with a friendly sign-off (e.g., "Cheers")

2. Tone:
   - Keep it warm, friendly, and conversational
   - Use contractions (e.g., "I’m", "you’re") for informality

3. Content:
   - Include specific event details to make the invitation clear
   - Add a personal touch (e.g., shared memories or excitement)
   - Invite a reply to confirm attendance
...
''',
  ...
  Future<void> _initStorage() async {
    try {
      ...
      _letterFile = File('$_storagePath/informal_letter_1.json');
    } catch (e) {
  ...
  int _parseLessonId(dynamic id) {
    ...
    if (id is String) {
      ...
      return match != null ? int.parse(match.group(0)!) : 8; // Align with intId
    }
    return 8; // Default to match informal_letter_1 intId
  }
  ...
  double _calculateScore(String text) {
    ...
    final wordScore = (wordCount / 80).clamp(0.0, 1.0) * 3;
  ...
  String _generateFeedback(double score) {
    if (score >= 8.0) {
      return "Excellent invitation! Your letter is warm, clear, and includes engaging event details.";
    } else if (score >= 6.0) {
      return "Good effort. Include more specific event details or a more personal touch.";
    } else {
      return "Needs improvement. Use a friendlier tone and provide clear event details.";
    }
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: Text(
          'IELTS Writing: Informal Letter 1',
